...
class CPAPIngestion:
    def __init__(self, reset=False):
        ...
        if not os.path.exists(PROCESSED_DIR):
            ...
            print("Limpando dados existentes (Reset)...")
    ...
    def read_str_edf(self, file_path):
        ...
        try:
            ...
            # Garante que start_time seja aware de timezone se necessário, ou converte para naive para o pandas
            if start_time is not None:
                start_time = start_time.replace(tzinfo=None)
            else:
                start_time = datetime(2000, 1, 1) # Fallback

            # Cada amostra representa um intervalo de tempo definido pela sfreq do arquivo
            # Usamos raw.times para obter o deslocamento em segundos de cada amostra
            df['data_terapia'] = [start_time + timedelta(seconds=t) for t in raw.times]
            
            # Arredondar para o dia para facilitar agrupamentos, se desejado, 
            # mas manter o timestamp original para evitar duplicatas em chaves primárias
            # df['data_terapia_dia'] = df['data_terapia'].dt.date
    ...
    def process_all_users(self):
        ...
        for user in users:
            ...
            # 1. Processar Sumário (STR.edf)
            str_path = os.path.join(user_path, "STR.edf")
            if os.path.exists(str_path):
                print(f" - Lendo sumário estatístico (STR.edf)")
                ...
                print(f" - [AVISO] STR.edf não encontrado para {user}")
    ...
    def save_to_parquet(self, df, table_name, dedupe_keys):
        ...
        # Processamos cada paciente individualmente para garantir integridade das partições
        for patient in df['patient'].unique():
            ...
            if os.path.exists(partition_path) and any(os.scandir(partition_path)) and not self.reset:
                # Carrega dados existentes do paciente
                df_existing = pd.read_parquet(partition_path)
                # Garante que a coluna de partição existe no df carregado (read_parquet em partição única pode omiti-la)
                ...
                if diff > 0:
                    print(f" - Usuário {patient}: {diff} novos registros adicionados.")
                    # Sobrescreve a partição com os dados consolidados
                    shutil.rmtree(partition_path)
                    table = pa.Table.from_pandas(df_combined)
                    pq.write_to_dataset(table, root_path=output_root, partition_cols=['patient_slug'])
                else:
                    print(f" - Usuário {patient}: Nenhum dado novo encontrado.")
            else:
                # Nova carga para este paciente
                print(f" - Usuário {patient}: {len(patient_new_data)} registros (nova carga).")
